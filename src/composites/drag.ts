...
export default (el: HTMLElement, container: HTMLElement) => {
  const ease = 0.3
  ...
  const tap = (e: MouseEvent | TouchEvent): boolean => {
    pressed = true
    curInertiaX = 0
    curInertiaY = 0
    curVelX = 0
  ...
  const drag = (e: MouseEvent | TouchEvent): boolean => {
    if (pressed) {
      let x = xpos(e)
      let y = ypos(e)
  ...
  const release = (e: MouseEvent | TouchEvent): boolean  => {
    if (pressed) {
      pressed = false
